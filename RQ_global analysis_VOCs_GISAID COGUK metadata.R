...
pacman::p_load(devtools, GISAIDR, nnet, splines,mclogit,emmeans,readr,ggplot2,ggthemes,scales,archive,dplyr,stringr,lubridate,lubridate,tidyr,countrycode,memoise,readxl,covidregionaldata,tidyquant,data.table,R.utils,locatexec,pals,marginaleffects,inspectdf,zoo,RSelenium)
...
target_dir = "/home/rodrigo/git/LineageExplorer/data/GISAID" # target download directory GISAID data
suppressWarnings(dir.create(file.path("plots",plotdir)))
tag = paste("@rquiroga777 based on work by @TWenseleers\n",today)
...
if (use_coguk) { GISAID = download_COGUK_meta()
...
# DEFINE VARIANT LINEAGES OF INTEREST *** TOM ***
if (lineages=="Tom") {system.time(GISAID$variant <- case_when(
...
# DEFINE VARIANT LINEAGES OF INTEREST *** RODRIGO ***
if (lineages=="Rodrigo") {system.time(GISAID$variant <- case_when(
  ...
  linplus("B.1.177")&
    datefrom("2020-05-27") ~ "B.1.177 (EU1)",
  TRUE ~ "Other" # assigns NA to remaining Unassigned & remove them later on
  # TRUE ~ "Other" # alternative: to assign Unassigned lineages to category Other
)) # 97s - note: could be sped up by using multidplyr & parallelization
}
...
ggsave(file=file.path("plots", plotdir,paste0("global_analysis_muller_plot_raw_data_all_countries_pooled",today,".png")), width=7, height=5)
...
qplot(data=meffects_sel1, 
  ...
  ggtitle("GROWTH RATE ADVANTAGE OF SARS-CoV2 VARIANTS",
          subtitle=paste0("based on multinomial spline fit variant ~ ns(date, df=2)+ns(date, df=2):continent+country,\nGISAID & COG-UK data, using data from countries with >=5 BQ.1*, BA.2.3.20 or BA.2.75.2\n(",
                          paste0(sel_countries[1:1], collapse=", "), "\n",
...
ggsave(file=file.path("plots", plotdir, paste0("global_multinom_fit_all_data_predictions_logit_scale_",today,".png")), width=20, height=12)
...
ggsave(file=file.path("plots", plotdir, paste0("global_multinom_fit_all_data_predictions_last6months_logit_scale_",today,".png")), width=20, height=12)
